  ...
  &.open {
    ...
    .end {
      transition: width 100ms ease-in-out,
        transform 300ms cubic-bezier(0.54, -0.81, 0.57, 0.57) 50ms;
    }
  }
`;
...
const Line = styled.div`
  height: 3px;
  transition: width 100ms ease-in-out,
    transform 250ms cubic-bezier(0.54, -0.81, 0.57, 0.57) 50ms;
`;

const MenuButton = ({ isOpen, onClick }) => (
  <Button
    className={`menu-button relative flex flex-col justify-between text-white text-opacity-75 h-6 -top-1 transform rotate-0 ml-auto z-9999 cursor-pointer ${
      isOpen ? 'open -rotate-45' : 'w-8'
    }`}
    onClick={onClick}
  >
    <Line
      className={`start bg-white bg-opacity-75 rounded origin-right transform ${
        isOpen ? 'w-1/2 -rotate-90 top-0.5' : 'w-3/5 top-0'
      }`}
    />
    <Line className="bg-white bg-opacity-75 w-full rounded transition-transform duration-300 ease-out" />
    <Line
      className={`end bg-white bg-opacity-75 self-end rounded origin-left transform ${
        isOpen ? 'w-1/2 -rotate-90 -top-0.5' : 'w-3/5 top-0'
      }`}
    />
  </Button>
...
const MenuPanel = ({ isOpen }) => (
  <Panel
    className={`fixed flex flex-col bg-base-100 text-left items-center w-screen h-screen top-0 right-0 p-6 opacity-0 z-min${
      isOpen ? ' open' : ''
    }`}
  >
  ...
  <nav className="hidden sm:block right-0 sm:right-10 ml-auto">
    <ul className="menu grid grid-cols-4 gap-x-1 relative bg-transparent w-full font-title font-medium text-right list-reset m-0 z-9999">
      {links.map((item) => (
        <li
          key={item.name}
          className="menu-item inline-block text-gray-1000 text-opacity-90 text-lg text-center font-normal w-19 max-w-24 py-0 px-0 last:pr-0 cursor-pointer z-9999"
        >
          <PageLink
    ...
    >
      <Fade direction="down" delay={250} triggerOnce>
        <div className="flex relative w-5/6 md:w-9/10 h-16 flex-wrap items-center justify-between top-0 mx-auto pt-6 pb-4 px-0 box-border">
          {showLogo ? <Logo className="logo-container" link="/" /> : null}
      ...
      {isMobile ? <MenuPanel isOpen={isOpen} /> : null}
    </div>
  );
};
Nav.defaultProps = {
